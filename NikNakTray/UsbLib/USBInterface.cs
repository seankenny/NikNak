...
namespace USBInterface
{
    internal class HIDInterface
    {
        ...
        //static extern bool ReadFileEx(IntPtr hFile, byte[] lpBuffer,
        //   uint nNumberOfBytesToRead, ref System.Threading.NativeOverlapped lpOverlapped,
        ...
        [DllImport("hid.dll", SetLastError = true)]
        static public extern bool HidD_GetPreparsedData(
        ...
        public static interfaceDetails[] getConnectedDevices()
        {
            ...
            while (b)
            {
                ...
                if (utilities.utilities.stringIsInteger(SN))
        ...
        public HIDInterface(ushort VID, ushort PID, ushort serialNumber, bool useAsyncReads)
        {
            ...
            if (!deviceConnected)
            {
                string hexVID = utilities.utilities.numToHexString(VID);
                string hexPID = utilities.utilities.numToHexString(PID);
        ...
        /// <param name="useAsyncReads">True - Read the device and generate events on data being available</param>
        public HIDInterface(string devicePath, bool useAsyncReads)
        {
        ...
        //This read function will be used with asychronous operation
        ...
        private void GetInputReportData(IAsyncResult ar)
        {
            ...
            //TODO handle exception with PCB is reset
